...
namespace StepManiaEditor
{
	...
	internal sealed class EditorSong : Notifier<EditorSong>, Fumen.IObserver<WorkQueue>
	{
		...
		/// </summary>
		private readonly Dictionary<ChartType, List<EditorChart>> Charts = new Dictionary<ChartType, List<EditorChart>>();
		/// <summary>
		...
		/// </summary>
		private readonly List<Chart> UnsupportedCharts = new List<Chart>();
		/// <summary>
		...
		private EditorImageData Banner;
		...
		private EditorImageData Background;
		...
		private EditorImageData Jacket;
		...
		private EditorImageData CDImage;
		...
		private EditorImageData DiscImage;
		...
		private EditorImageData CDTitle;
		...
		public IReadOnlyList<EditorChart> GetCharts(SMCommon.ChartType chartType)
		{
		...
		public void Save(FileFormatType fileType, string fullPath, Action callback)
		{
			...
			WorkQueue.Enqueue(new Task( () =>
			{
				...
				// Save all EditorCharts to Charts and call onAllChartsCompelte when done.
				...
				if (numCharts > 0)
				{
					...
					editorCharts[0].SaveToChart((Chart savedChart, Dictionary<string, string> savedChartProperties) =>
					{
						OnChartSaved(song, customProperties, editorCharts, savedChart, savedChartProperties, 0, numCharts, onAllChartsComplete);
					});
				}
				else
				{
			...
			Action onAllChartsComplete)
		{
			...
			{
				editorCharts[nextChartIndex].SaveToChart( (Chart savedChart, Dictionary<string, string> savedChartProperties) =>
				{
					OnChartSaved(
						song,
						customProperties,
